					...
					struct feature*** nbrs, int max_nn_chks )
{
	...
	_nbrs = reinterpret_cast<feature**>(calloc( k, sizeof( struct feature* )));
	min_pq = minpq_init();
	...
	while( min_pq->n > 0  &&  t < max_nn_chks )
	{
		...
		for( i = 0; i < expl->n; i++ )
		{
			...
			bbf_data =reinterpret_cast<struct bbf_data*>( malloc( sizeof( struct bbf_data ) ));
			if( ! bbf_data )
			{
	...
	for( i = 0; i < n; i++ )
	{
		bbf_data =reinterpret_cast<struct bbf_data*>(  _nbrs[i]->feature_data );
						   ...
						   CvRect rect, int model )
{
	...
	sp_nbrs = reinterpret_cast<feature**>( calloc( k, sizeof( struct feature* ) ));
	for( i = 0; i < n; i++ )
	{
		if( model )
...
struct kd_node* kd_node_init( struct feature* features, int n )
{
	...
	kd_node = reinterpret_cast<struct kd_node*>( malloc( sizeof( struct kd_node )) );
