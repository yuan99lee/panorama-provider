				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	octvs = log( static_cast<double>(MIN( init_img->width, init_img->height )))
			/ log(2.0) - 2;
...
prior to scale space construction.*/
	...
	//如果有多个主方向，则根据lower论文的section 5计算出一个权威点
	calc_feature_oris( features, gauss_pyr );
	//根据lower的section 6为每个关键点计算描述符
	...
	//清理资源
	n = features->total;
	*feat = reinterpret_cast<struct feature*>(calloc( n, sizeof(struct feature)));
	*feat = reinterpret_cast<struct feature*>(cvCvtSeqToArray( features, *feat, CV_WHOLE_SEQ));
	for( i = 0; i < n; i++ )
	{
...
@param octvs number of octaves of scale space 在尺度上的倍数的数量（即金字塔的层数）
@param intvls number of intervals per octave 每倍的间距
...
{
	IplImage*** gauss_pyr;
	double* sig = reinterpret_cast<double*>(calloc( intvls + 3, sizeof(double)));
	double sig_total, sig_prev, k;
	int i, o;

	gauss_pyr = reinterpret_cast<IplImage***>(calloc( octvs, sizeof( IplImage** )));
	for( i = 0; i < octvs; i++ )
		gauss_pyr[i] = reinterpret_cast<IplImage**>(calloc( intvls + 3, sizeof( IplImage* )));
...
IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
	...
	dog_pyr = reinterpret_cast<IplImage***>(calloc( octvs, sizeof( IplImage** )) );
	for( i = 0; i < octvs; i++ )
		dog_pyr[i] =  reinterpret_cast<IplImage**>(calloc( intvls + 2, sizeof(IplImage*)));
						   ...
						   CvMemStorage* storage )
{
	...
	for( o = 0; o < octvs; o++ )
		for( i = 1; i <= intvls; i++ ) //遍历每张图，两个边缘不检测
			for(r = SIFT_IMG_BORDER; r < dog_pyr[o][0]->height-SIFT_IMG_BORDER; r++)
				for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c++)
					/* perform preliminary check on contrast */
					if( ABS( pixval32f( dog_pyr[o][i], r, c ) ) > prelim_contr_thr )
						 //再比较周围的26个点（立体的，金字塔上下层，以及本层的3×3矩阵）
						 //共3×3×3-1=26个点
						if( is_extremum( dog_pyr, o, i, r, c ) )//检测是不是26个点中最小或最大
						{
							...
							if( feat )
							{
								...
								if( ! is_too_edge_like( dog_pyr[ddata->octv][ddata->intvl],
									ddata->r, ddata->c, curv_thr ) ) //是否太像边缘
								{
...
struct feature* new_feature( void )
{
	...
	feat =  reinterpret_cast<struct feature*>(malloc( sizeof( struct feature )));
	memset( feat, 0, sizeof( struct feature ) );
	ddata = reinterpret_cast<struct detection_data*>(malloc( sizeof( struct detection_data )));
...
double* ori_hist( IplImage* img, int r, int c, int n, int rad, double sigma)
{
	...
	hist = reinterpret_cast<double*>(calloc( n, sizeof( double )));
					 ...
					 double scl, int d, int n )
{
	...
	hist = reinterpret_cast<double***>(calloc( d, sizeof( double** )));
	for( i = 0; i < d; i++ )
	{
		hist[i] = reinterpret_cast<double**>(calloc( d, sizeof( double* )));
		for( j = 0; j < d; j++ )
			hist[i][j] = reinterpret_cast<double*>(calloc( n, sizeof( double )));
	}
	...
	radius = static_cast<int>(hist_width * sqrt(2.0) * ( d + 1.0 ) * 0.5 + 0.5);
