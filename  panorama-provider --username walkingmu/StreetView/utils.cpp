...
char* replace_extension( const char* file, const char* extn )
{
	...
	new_file = reinterpret_cast<char*>(calloc( strlen( file ) + strlen( extn ) + 2,  sizeof( char )));
...
char* basename( const char* pathname )
{
	...
	last_slash = const_cast<char*>(strrchr( pathname, '/' ));
	if( ! last_slash )
	{
		base = reinterpret_cast<char*>(calloc( strlen( pathname ) + 1, sizeof( char )));
		strcpy( base, pathname );
	}
	else
	{
		base = reinterpret_cast<char*>(calloc( strlen( last_slash++ ), sizeof( char )));
