...
struct feature*** matched )
{
	...
	_matched = reinterpret_cast<struct feature**>(calloc( n, sizeof( struct feature* )));
	for( i = 0; i < n; i++ )
		if( get_match( features + i, mtype ) )
		{
			rdata = reinterpret_cast<struct ransac_data*>(malloc( sizeof( struct ransac_data )));
									...
									int m, gsl_rng* rng )
{
	...
	sample = reinterpret_cast<struct feature**>(calloc( m, sizeof( struct feature* )));
	for( i = 0; i < m; i++ )
	{
		do
		{
						 ...
						 CvPoint2D64f** pts, CvPoint2D64f** mpts )
{
	...
	_pts = reinterpret_cast<CvPoint2D64f*>(calloc( n, sizeof( CvPoint2D64f )));
	_mpts = reinterpret_cast<CvPoint2D64f*>(calloc( n, sizeof( CvPoint2D64f )));
				   ...
				   struct feature*** consensus )
{
	...
	_consensus =  reinterpret_cast<struct feature**>(calloc( n, sizeof( struct feature* )));
